...
cargo tree -p knf-merge --depth 1       # verify the core's dependency boundary
```
...
`Value` → append `--set` layers → `merge_all` over the flat list → `format::emit`.
...
- **Null is an ordinary value, not a delete instruction.** This is what makes
  ...
  `crates/knf-merge/tests/props.rs`.
...
- `crates/knf-merge/tests/cases.rs` is table-driven; adding a merge case is one line
  ...
  on the binary crate).
- `crates/knf-merge/tests/props.rs` holds the proptest invariants above. Its value
