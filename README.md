...
This installs the `knf` executable and the `knf` Python module (see
[Python](#python)). Wheels are published for CPython 3.9+ on Linux (glibc and
musl) on x86-64 and ARM64, macOS on Intel and Apple Silicon, and Windows on x64
and ARM64. No Rust toolchain is needed to install a wheel.
...
`deep_merge` runs the same merge as the command line, natively, and returns a
`dict`:
...
A file that can't be read, parsed or merged raises `knf.KnfError`. TOML datetimes
come back as their TOML spelling in a `str`, which is also what `knf -f json` prints.
