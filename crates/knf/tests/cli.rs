...
fn ambiguous_group_error() {
    let dir = tree(&[
        ...
        ("config/server/apache.toml", "[server]\nkind = \"apache\"\n"),
        ("config/server/nginx.toml", "[server]\nkind = \"nginx\"\n"),
    ]);
    insta::assert_snapshot!(run_err(&dir, &["matrix", "config/"]));
}

/// §4.1: one path must not mean two things, so the default command points at
...
fn matrix_max_fails_before_writing() {
    let dir = tree(&[
        ("config/db/mysql.toml", "b = 1\n"),
        ("config/db/postgres.toml", "b = 2\n"),
        ("config/server/apache.toml", "c = 1\n"),
        ("config/server/nginx.toml", "c = 2\n"),
    ]);
    let err = run_err(
        &dir,
        &["matrix", "config/", "--out-dir", "out/", "--max", "3"],
    );
    assert!(err.contains("--max"), "{err}");
    assert!(
        !dir.path().join("out").exists(),
        "wrote before checking max"
    );
}
...
fn matrix_over_singletons_is_a_plain_merge() {
    let dir = tree(&[
        ("config/base.toml", "a = 1\nb = 1\n"),
        ("config/db/only.toml", "b = 2\n"),
    ]);
    assert_eq!(run(&dir, &["matrix", "config/"]), "a = 1\nb = 2\n");
}
