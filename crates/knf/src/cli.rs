...
expression with `+` (or --output-separator), used as-is. A --set expression
containing a path separator or NUL byte is rejected."
    )]
    pub output_dir: Option<PathBuf>,

    /// Separator between factor names in Cartesian-product output files
    #[arg(
        long,
        value_name = "SEP",
        long_help = "\
Separator between factor names in Cartesian-product output files.
Defaults to `+`. Valid only in Cartesian-product mode."
    )]
